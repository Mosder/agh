...
#
# Na start trzeba skonfigurować środowisko wirtualne i zależności. Korzystaj z Pythona 3.11 lub 3.12.
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    exp_pred_train = np.expm1(pred_train)
    exp_pred_test = np.expm1(pred_test)
    # calculate train and test RMSE
    rmse_train = root_mean_squared_error(exp_y_train, exp_pred_train)
    rmse_test = root_mean_squared_error(exp_y_test, exp_pred_test)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Występuje tutaj przeuczenie, gdyż dane RMSE danych testowych jest większe o prawie 30%. Sam błąd też jest dość duży w porównaniu do mieszkań, z których większość zawiera się w przedziale 100 do 300 tysięcy.
...
lasso_cv = LassoCV(alphas=1000, random_state=0)

ridge_cv.fit(X_train, y_train)
lasso_cv.fit(X_train, y_train)

reg_ridge_alpha = ridge_cv.alpha_
reg_lasso_alpha = lasso_cv.alpha_

print("Ridge:")
ridge_train_rmse, ridge_test_rmse = assess_regression_model(ridge_cv, X_train, X_test, y_train, y_test)
print(f"alpha: {reg_ridge_alpha:.5f}\n")
print("Lasso:")
lasso_train_rmse, lasso_test_rmse = assess_regression_model(lasso_cv, X_train, X_test, y_train, y_test)
print(f"alpha: {reg_lasso_alpha:.5f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Widać, że użycie Cross Validation bardzo zmniejszyło overfitting. Różnica dla RidgeCV wynosi ok. 1500 dolarów, a dla LassoCV jest to jedynie 500 dolarów.
...
df = df.replace({"education": education_mapping, "contact": contact_mapping, "month": month_mapping,
                 "day_of_week": day_of_week_mapping, "y": y_mapping})
y = df.pop('y')
df
...
df_nan.plot.bar(title="Missing values")
df_nan
...
def assess_binary(model, X_train, y_train, X_test, y_test):
    # train and predict
...
def print_metrics(model_str, precision, recall, f1):
    print(f"Model: {model_str}")
    print(f"Precision: {precision*100:.5f}%")
    print(f"Recall: {recall*100:.5f}%")
    print(f"F1: {f1*100:.5f}%")

log_reg = LogisticRegression(penalty=None, class_weight="balanced", random_state=0)
nol_precision, nol_recall, nol_f1 = assess_binary(log_reg, X_train, y_train, X_test, y_test)
print_metrics("Logistic w/o regularization", nol_precision, nol_recall, nol_f1)
print()

log_reg_cv_l2 = LogisticRegressionCV(Cs=100, cv=5, scoring='f1', class_weight="balanced", random_state=0, n_jobs=-1)
l2_precision, l2_recall, l2_f1 = assess_binary(log_reg_cv_l2, X_train, y_train, X_test, y_test)
print_metrics("Logistic CV L2", l2_precision, l2_recall, l2_f1)
print()

log_reg_cv_l1 = LogisticRegressionCV(Cs=100, cv=5, scoring='f1', class_weight="balanced", random_state=0, n_jobs=-1,
                                    penalty='l1', solver='saga')
l1_precision, l1_recall, l1_f1 = assess_binary(log_reg_cv_l1, X_train, y_train, X_test, y_test)
print_metrics("Logistic CV L1", l1_precision, l1_recall, l1_f1)
...
print(f"F1 train: {100*f1_train:.5f}%")
print(f"F1 test: {100*f1_test:.5f}%")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Różnica w metryce F1 pomiędzy zbiorem treningowym, a testowym jest niewielka zatem nie występuje overfitting. Można jednak powiedzieć, że występuje underfitting, gdyż te wartości są dość małe.
#
# Różnice w metrykach są ledwie zauważalne pomiędzy modelem bez regularyzacji, a modelami z regularyzacją zatem uważam, że nie ma sensu w tym przypadku tworzenia modeli z regularyzacją.
#
# W naszym przypadku zdecydowanie interesuje nas bardziej czułość od precyzji, gdyż chcemy wysyłać reklamy do jak największej liczby ludzi, którzy byliby zainteresowani ofertą. Dużo więcej stracimy nie zyskując klienta poprzez nie wysłanie mu reklamy niż wysyłając reklamę do niezainteresowanej osoby.
...
numerical_pipeline = Pipeline([("poly", poly_features), ("scale", standard_scaler)])
...
log_reg = LogisticRegression(penalty=None, class_weight="balanced", random_state=0, max_iter=1000)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Jak widać po wartościach F1 na zbiorze treningowym oraz testowym, nie występuje tutaj overfitting. Z tego powodu nie ma sensu używać tutaj regularyzacji.
