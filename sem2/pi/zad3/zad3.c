...
void bernoulli_gen(int tab[], size_t n, double probab) {
    for (int i = 0; i < n; i++) tab[i] = rand() - probab < probab * RAND_MAX ? 1 : 0;
}
...
void two_dice_probab_distrib(double distrib[], int throws_num) {
    for (int i = 0; i < 11; i++) distrib[i] = 0;
    for(int i = 0; i < throws_num; i++) distrib[rand() % 6 + rand() % 6]++;
    for (int i = 0; i < 11; i++) distrib[i] /= throws_num;
}
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n) {
    // Nazwy zmiennych są niefortunne, bo valve >>>>> nintendo co nie jest przewidywanym wynikiem, ale co zrobisz
    int nintendo = 0;
    int valve = 0;
    for (int i = 0; i < n; i++) {
        if (rand() % 3 == rand() % 3) valve++;
        else nintendo++;
    }
    *p_switch_wins = nintendo;
    *p_nonswitch_wins = valve;
}
